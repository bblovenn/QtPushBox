...
void GameMap::Paint(QPainter *_p, QPoint _Pos)
{
    for(int i = 0;i<mRow;i++){
        for(int j=0;j<mCol;j++){
            ...
            QImage img(imgName);
            _p->drawImage( QRect(
                               _Pos.x() + j * img.width(),   // ① 绘制区域左上角 X 坐标：基准位置 + 列索引 × 图片宽度
                               ...
                               // 示例：_Pos.x()=100, img.width()=32, j=2
                               // X = 100 + 2×32 = 164

                               _Pos.y() + i * img.height(),  // ② 绘制区域左上角 Y 坐标：基准位置 + 行索引 × 图片高度
                               ...
                               // 示例：_Pos.y()=50, img.height()=32, i=1
                               // Y = 50 + 1×32 = 82
